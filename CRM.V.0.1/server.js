...
function normalizeLead(row) {
    ...
    const followUps = database.prepare('SELECT f.id, f.lead_id AS leadId, f.type, f.due_at AS dueAt, f.assigned_to AS assignedTo, s.name AS assignedEmployee, f.status, f.notes, f.created_by AS createdBy, f.created_at AS createdAt, f.completed_at AS completedAt, f.completed_by AS completedBy, f.outcome, f.missed_reason AS missedReason FROM follow_ups f LEFT JOIN staff s ON s.id = f.assigned_to WHERE f.lead_id = ? ORDER BY datetime(f.due_at) DESC').all(row.id);
...
function stageMissing(lead, body) {
    ...
    if (lead.stage === 'New') {
        ...
        if (!survey) missing.push('Survey ID', 'Survey date', 'Survey time', 'Surveyor', 'Site address', 'Survey status');
            ...
            if (survey.status !== 'Completed') missing.push('Survey not completed');
        }
...
function completeStage(user, lead, body) {
    ...
    try {
        ...
        const details = (() => { try { return lead.details_json ? JSON.parse(lead.details_json) : {}; } catch (error) { return {}; } })();
        const updatedDetails = { ...details, ...body, ...(nextStage === 'Lost' ? { lostReason: body.lostReason, lostDate: timestamp, lostBy: user.id } : {}) };
...
function markOverdueFollowUps() {
    database.prepare(`UPDATE follow_ups SET status = 'Overdue' WHERE status = 'Scheduled' AND datetime(due_at) < datetime('now')`).run();
}
...
async function handleApi(request, response, url) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/dashboard') {
        ...
        return json(response, 200, {
            metrics: {
                ...
                myLeads,
                todaysFollowUps: count(`SELECT COUNT(*) AS count FROM follow_ups WHERE lead_id IN (${placeholders}) AND date(due_at) = ? AND status = 'Pending'`, [...ids, today]),
    ...
    if (request.method === 'GET' && url.pathname === '/api/dashboard/groups') {
        ...
        const groups = {
            todaysFollowUps: query(`SELECT f.id, l.id AS lead_id, l.customer_name, l.mobile_number, time(f.due_at) AS due_time FROM follow_ups f JOIN leads l ON l.id = f.lead_id WHERE l.id IN (${placeholders}) AND date(f.due_at) = ? AND f.status = 'Pending' AND l.stage NOT IN ('Lost', 'Completed') AND l.status = 'Active' ORDER BY datetime(f.due_at)`, [...ids, today]),
    ...
    if (request.method === 'POST' && url.pathname === '/api/leads') {
        ...
        database.exec('BEGIN');
        try {
            database.prepare(`INSERT INTO leads (id, customer_name, mobile_number, email, lead_date, lead_source, assigned_to, stage, priority, location, created_by, created_at, updated_at, details_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(id, body.customerName.trim(), body.mobileNumber.trim(), body.email || null, body.leadDate, body.leadSource, assignedEmployee.id, 'New Lead', body.leadPriority || 'Warm', details.city || body.location || null, user.id, timestamp, timestamp, JSON.stringify(details));
    ...
    if (leadActionMatch && request.method === 'POST') {
        ...
        if (action === 'schedule-survey') {
            if (lead.stage !== 'Opportunity') return json(response, 422, { error: 'A survey can only be scheduled from the Opportunity stage.' });
            ...
            database.prepare('INSERT INTO surveys (id, lead_id, survey_date, assigned_to, customer, address, sanctioned_load, electricity_details, roof_information, recommended_capacity, remarks, status, survey_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(surveyId, leadId, `${body.date}T${body.time}`, body.surveyor, lead.customer_name, body.address, 'Pending', 'Pending', 'Pending', 'Pending', body.remarks, 'Scheduled', body.type);
            audit(user, 'Scheduled', 'survey', surveyId, { date: body.date, time: body.time, description: `Site survey scheduled for ${body.date} at ${body.time}.` });
            notify(body.surveyor, 'survey-assigned', `Survey ${surveyId} assigned for lead ${leadId}.`, 'lead', leadId);
            return json(response, 201, { surveyId });
        }
        if (action === 'document') {
    ...
    if (followUpMatch && request.method === 'POST') {
        ...
        const id = crypto.randomUUID();
        database.prepare('INSERT INTO follow_ups (id, lead_id, due_at, type, assigned_to, status, notes, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .run(id, leadId, `${body.date}T${body.time}`, body.type, body.assignedTo, 'Scheduled', body.notes || null, user.id, now());
    ...
    if (completeFollowUpMatch && request.method === 'POST') {
        ...
        database.prepare('UPDATE follow_ups SET status = ?, notes = ?, outcome = ?, completed_at = ?, completed_by = ? WHERE id = ?').run('Completed', `${body.remarks.trim()} | Next action: ${body.nextAction.trim()}`, body.outcome || body.customerResponse || null, now(), user.id, followUpId);
